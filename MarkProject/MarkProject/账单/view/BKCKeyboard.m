...
+ (instancetype)init {
    BKCKeyboard *view = [BKCKeyboard loadFirstNib:CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_WIDTH / 5 * 4 + TabbarSafeBottomMargin)];
    [view setHidden:YES];
    [view initUI];
    return view;
}
...
//    [self borderForColor:kColor_BG borderWidth:1.f borderType:UIBorderSideTypeTop];
    ...
    [self.nameLab setFont:[UIFont systemFontOfSize:14]];
    [self.nameLab setTextColor:[UIColor grayColor]];
    [self.markField setFont:[UIFont systemFontOfSize:14]];
    [self.moneyLab setFont:[UIFont systemFontOfSize:18]];
    [self.markField setFont:[UIFont systemFontOfSize:14]];
    [self.markField setTintColor:TintColor];
    [self.markField setTextColor:[UIColor grayColor]];
    
    [self.textConstraintH setConstant:60];
    [self.keyConstraintB setConstant:TabbarSafeBottomMargin];
    ...
    for (id obj in self.subviews) {
        if ([obj isKindOfClass:[UIButton class]] && [obj tag] >= 10) {
            ...
            if (btn.tag == FINISH_TAG) {
                [btn setBackgroundImage:[UIImage imageWithColor:[MDInstance sharedInstance].themeColor] forState:UIControlStateNormal];
...
//        [self setTop:SCREEN_HEIGHT - self.height];
        self.Easy_top = SCREEN_HEIGHT - self.height;
...
//        [self.textContent setTop:(self.height - keyHeight) - countcoordinatesX(60)];
        self.textContent.lx_top = (self.height - keyHeight) - 60;
    } completion:^(BOOL finished) {
